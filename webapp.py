"""
Turn rest_api_server.py into webapp.py
...
from bottle import run, get, post, view, abort
from bottle import response, request, static_file, template
import pubsub
import pubsub as comb
import session # it's needed!
import time


secret = 'question is the average life expectancy of a stark or targaryen is short...'
User = str
...
def show_main_page(user=None):
    ...
    if user is None:  # try user again after reading from the cookie
        return template('login', null=None)  # to check what is null
